...
await asyncExec(`npx prisma migrate deploy --schema src/prisma/schema.prisma`, {
...
afterAll(async () => {
  ...
  describe('Scenario : repository.findById', () => {
    it('should find a webinar by id', async () => {
      ...
      expect(foundWebinar?.props.id).toBe('webinar-id');
      expect(foundWebinar?.props.organizerId).toBe('organizer-id');
      expect(foundWebinar?.props.title).toBe('Webinar title');
      expect(foundWebinar?.props.seats).toBe(100);
    });
  ...
  describe('Scenario : repository.update', () => {
    it('should update a webinar', async () => {
      ...
      expect(maybeWebinar?.seats).toBe(200);
    });
  });
});
